...
class RailstutorialController < ApplicationController
	...
	def process_students
		csv_array = []
    	csv_array<< ['Student', 'ID', 'SIS User ID', 'SIS Login ID', 'Section','score', 'GitHub Username','Number of Commits','Number of Branches','Heroku Live']
    	client = Octokit::Client.new login: params[:username], password: params[:password]

		Student.order("lower(name)").each do |student|
			score=0
			
			# CHECK GITHUB REPO
			if student.github_username != "" && student.github_username != nil
			...
			# CREATE SPREADSHEET
			csv_array << [student.name, student.canvasID, student.SISid,  student.niner_net, student.section, score, student.github_username,numberOfCommits,branchCount,heroku]
			@csv =""
